...
### Added
- **Year-to-date capital-gains tracker (€30k bracket)** — a new card on the Transaction History page shows how much realized capital gain has accumulated in the year versus the €30,000 Finnish capital-income bracket threshold, so it's easy to see how close the year is to the higher 34 % rate. New `GET /transactions/tax-calculations/summary?year=` endpoint aggregates the taxable gains (`luovutusvoitto`) from saved tax calculations and applies the 30 %/34 % bracket to the **combined** net gain (the threshold is a per-year total, not per sale). The card shows a progress bar, remaining 30 %-band headroom (or amount over the threshold), gains/losses breakdown, and estimated tax, with a year selector. It updates automatically when calculations are saved or deleted. Note: it only reflects sales with a saved calculation and excludes dividends/other capital income, which also count toward the same limit.
